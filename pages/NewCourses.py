        ...
        if st.button(subtopic):
            ...
            if res.status_code == 200:
                try:
                    ...
                    audio_button_key = f"Play_{subtopic}"
                    if st.button(f"Play {subtopic} Content", key=audio_button_key):
                        try:
                            tts = gTTS(text=content_message, lang='en')
                            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as fp:
                                tts.save(fp.name)
                                st.audio(fp.name, format="audio/mp3")
                                st.write("Audio is playing...")
                                st.write(f"Temporary audio file path: {fp.name}")
